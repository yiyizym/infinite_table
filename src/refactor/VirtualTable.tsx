...
class VirtualTable extends React.Component<VirtualTableProps, VirtualTableState> {
  ...
  private readonly delayedEvents: (ScrollHookOpts | Event)[];
  ...
  public componentWillUnmount(): void {
    // console.log('VirtualTable unmount')
  ...
  private update = (timeStamp: number): void => {
    ...
    if (flag & ScrollEvent.INIT) {
      ...
      return VirtualTable.setFixedStoreState(head, tail, top)
    }
  ...
  private static setFixedStoreState(head: number, tail: number, top: number): void {
    const leftFixedStore = Store.get(0 - getCurrentID())
    const rightFixedStore = Store.get(1 << 31 + getCurrentID())
    if (leftFixedStore && leftFixedStore.leftPointer) {
      leftFixedStore.leftPointer.setState({ head, tail, top })
    }
    if (rightFixedStore && rightFixedStore.rightPointer) {
      rightFixedStore.rightPointer.setState({ head, tail, top })
    }
  }
}
